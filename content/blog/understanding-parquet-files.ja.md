+++
title = "分析ワークロードでParquetがCSVに勝る理由"
date = 2026-01-23
...
ディスク上では、ファイルのデータは連続したバイト列として格納される。[^1] $[b_0, b_1, b_2, \ldots, b_n]$ で、各 $b_i \in \lbrace 0,1 \rbrace^8$、$n$ は分析ワークロードでは数百万（MB）から数十億（GB）が一般的だ。
...
10バイトから1MB（10万倍のデータ）に増えても、読み取るデータが連続していればI/O時間は2倍にもならない。目標は明確だ。**seekを最小化し、seekあたりのバイト数を最大化する**。これを実現する戦略が**バッチ処理**だ。ファイル中に散らばった小さな読み取りを多数行う代わりに、大きな連続チャンクを読み取る。
...
分析データは通常、表形式で、行と列がある。テーブルをバイト列にシリアライズするとき、2つの自然な選択肢がある。簡単な従業員テーブルを考えてみよう。
...
**行グループ**（各約128MB）は行の水平パーティションだ。これが並列処理を可能にする。SparkやBigQueryのような分散クエリエンジンは、異なる行グループを異なるワーカーに割り当てられる。
...
バイト数が減ればI/Oが速くなる。圧縮はプロジェクション効率の利点を増幅する。

これらの技術は各列チャンク内のページレベルで適用される。各チャンクには単一列の値だけが含まれるので、すべての値が同じ型を共有する。そして実際には、列内の値はパターンに従うことが多い（繰り返されるカテゴリ、連続するタイムスタンプ、ソートされたキー）。Parquetはこの両方を活用する。
...
他にも多くの技術がある（ビットパッキング、各種圧縮コーデック）が、核心となるアイデアは同じだ。**列ごとに値をグループ化すると、圧縮に適したパターンが現れる**。

[^3]: Parquetはデータをソートしない。書き込み前にソートしておく必要がある。RLEは主ソートキーにのみ効果的で、二次キーはランが短くなる。
...
`user_id` のような高カーディナリティの列では、最小/最大は役に立たない（範囲が全域をカバーしてしまう）。Bloomフィルターは別のアプローチを提供する。複数のハッシュ関数を持つビット配列で、「確実にここにはない」か「ここにあるかもしれない」を返す。偽陽性率は $(1 - e^{-kn/m})^k$ に従う。$k$ はハッシュ関数の数、$n$ は行グループの行数、$m$ はビット数だ。エレガントな閉形式の最適解が存在する。これは別の記事のテーマだ。

</details>

## トレードオフ

Parquetは分析的な読み取りに最適化されている。多くの行、少ない列というパターンだ。コストは2つの場所に現れる。
